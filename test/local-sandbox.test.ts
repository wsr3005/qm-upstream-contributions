...
test("sandbox containers always run through the amd64 runtime contract", async () => {
  const fake = installFakeDocker(daemonPort);
  const sb = makeSandbox(fake);
  const handle = await sb.provision(rw(scopeId("personal", "U-amd64")));
  const run = fake.commands.find((args) => args[0] === "run" && args.includes(handle.id));
  assert.ok(run);
  assert.deepEqual(run.slice(0, 4), ["run", "-d", "--platform", "linux/amd64"]);
  assert.equal(fake.containers.get(handle.id)?.labels["qm.sandbox-platform"], "linux/amd64");
  await sb.teardown(handle, { destroy: true });
});

test("a resident container without the amd64 runtime contract is replaced", async () => {
  const fake = installFakeDocker(daemonPort);
  const scope = scopeId("personal", "runtime-upgrade");
  const name = localContainerName(scope, "upgrade-org");
  const volume = localVolumeName(scope, "upgrade-org");
  fake.containers.set(name, {
    name,
    imageId: fake.imageId,
    running: false,
    labels: { "qm.org": "upgrade-org" },
    volume,
  });
  fake.volumes.add(volume);
  fake.volumeOwners.set(volume, "upgrade-org");
  const sb = makeSandbox(fake, { orgId: "upgrade-org" });
  const handle = await sb.provision(rw(scope));
  assert.equal(handle.id, name);
  assert.equal(fake.runCount, 1);
  assert.equal(fake.containers.get(name)?.labels["qm.sandbox-platform"], "linux/amd64");
  await sb.teardown(handle, { destroy: true });
});
...
test("a stopped legacy container anchors the source volume by immutable container id", async () => {
  ...
  await assert.rejects(sandbox.provision(rw(scope)), (error: Error) => {
    assert.match(error.message, /docker run --platform linux\/amd64/);
