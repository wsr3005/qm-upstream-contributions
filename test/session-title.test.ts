...
test("sanitizeTitle rejects reply-shaped output instead of truncating it into a title", async () => {
  ...
  assert.equal(sanitizeTitle("Sorry_ state handling"), "Sorry_ state handling");
  assert.equal(sanitizeTitle("Sure_ flag parsing"), "Sure_ flag parsing");
  assert.equal(sanitizeTitle("Unfortunately_ enum mapping"), "Unfortunately_ enum mapping");
  assert.equal(sanitizeTitle("I；think this is the issue"), undefined);
  assert.equal(sanitizeTitle("I…think this is the issue"), undefined);
  assert.equal(sanitizeTitle("I、think this is the issue"), undefined);
  assert.equal(sanitizeTitle("I（think this is the issue"), undefined);
