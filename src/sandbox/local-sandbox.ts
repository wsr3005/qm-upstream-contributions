...
export function createLocalSandbox(workspace: WorkspaceStore, opts: LocalSandboxOptions = {}): Sandbox {
  ...
  async function containerState(name: string): Promise<{ running: boolean; imageId: string } | null> {
    const r = await dexec([
      "inspect",
      "-f",
      `{{.State.Running}} {{.Image}} {{if .Config.Labels}}{{index .Config.Labels "qm.org"}}{{end}}`,
      name,
    ]);
    if (r.code !== 0) {
    ...
    const [running = "", imageId = "", owner = ""] = r.stdout.trim().split(/\s+/);
    ...
    return provisionQueue(scope, async () => {
      ...
      if (legacyExists) {
        ...
        if (!hadVolume) {
          ...
          const copy = `docker run --rm --network none --entrypoint sh --volumes-from ${shq(`${legacyContainerId}:ro`)} -v ${shq(`${volume}:/to`)} ${shq(image)} -c ${shq("cp -a /root/. /to/")}`;
