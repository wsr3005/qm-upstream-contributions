...
test("POST /api/turn forwards blobId attachments to core as references (no inline bytes)", async () => {
  ...
  const r = await fetch(`${base}/api/turn`, {
    ...
    body: JSON.stringify({
      ...
      qaReservation: "signed-reservation-1",
      attachments: [
  ...
  assert.equal(lastTurnBody!.qaReservation, "signed-reservation-1");
});
  ...
  for (const [user, text] of [
    ...
    const r = await fetch(`${base}/api/turn`, {
      ...
      body: JSON.stringify({
        ...
        qaReservation: `signed-reservation-${user}`,
      }),
    });
    assert.equal(r.status, 200);
...
test("POST /api/turn rejects an unsigned QA idempotency key before reaching core", async () => {
  setTurnBody(null);
  const r = await fetch(`${base}/api/turn`, {
    method: "POST",
    headers: { ...IDENTITY, "content-type": "application/json" },
    body: JSON.stringify({ text: "msg", threadRef: "web:alice:t-unsigned", idempotencyKey: "qa-unsigned" }),
  });
