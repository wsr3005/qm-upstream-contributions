...
const apiRoutes: readonly WebRoute[] = [
  ...
  },
  {
    ...
    handle: async (c) => {
      ...
      const fingerprint = idempotencyKey ? createHash("sha256").update(JSON.stringify(baseTurn)).digest("hex") : null;
      const turn = idempotencyKey
        ? { ...baseTurn, idempotencyKey: `web-qa:${encodeURIComponent(user)}:${idempotencyKey}:${fingerprint}` }
        : baseTurn;
