...
The root object requires `contract: 1`, `orgId`, `publicUrl`, `target`, and `services` including `core`. Docker and Fly also require `sandbox.app`. Docker may select `sandbox.backend: "local"` with a digest-pinned `sandbox.image`; Core then reaches the host Docker Engine and boots that exact image. This grants Core control of that Docker host through its local Unix socket, so use it only on a trusted single-tenant host, not as a hardened multi-tenant boundary; remote and TCP Docker contexts are rejected. A pre-organization local volume is never silently replaced: provisioning stops if its old container is still running, then supplies a recovery command only while that stopped container anchors the expected old home read-only by immutable container ID. The command uses the configured digest-pinned sandbox image with networking disabled, cleans up a failed copy, removes the old writer before marking a successful copy, and finally retires the old volume. An orphaned old volume fails closed for manual recovery instead of risking Docker implicitly creating an empty source. If the operator process stops between those steps, later provisioning rejects an unmarked destination, an untrusted completion that still has its old writer, or an unretired old home instead of exposing partial or supposedly deleted data. Destroy records an organization-owned marker before removing the container, network, home, and migration marker; a restarted Core finishes that deletion before allowing a new provision. Docker purge deletes data volumes strictly before destroy markers and fails while any data volume remains in use. On AWS the sandbox substrate is an explicit choice: omitting the `sandbox` block runs named Lambda MicroVM images; declaring one requires `sandbox.backend` — `"sprites"` boots the operator-published layer image in `sandbox.app`, `"aws"` states the MicroVM default in the file. Unknown contract majors fail closed. `target` is `docker`, `fly`, or `aws`.

Common optional fields select the model, plugins, extra skill directories, per-service non-secret environment values, image overrides, sandbox settings, and an external security screen. `botName` (at most 31 characters, so the generated "`<botName>` SSO" app name fits Slack's 35-character cap) names the bot everywhere users see it — the generated Slack app manifests, the prompt identity, and sign-in pages — and `orgName` (at most 40) is how the bot refers to the organization; both default to neutral values and can be changed live from the Admin page's Branding card, which then takes precedence over the deployed values. `sandbox.backend` selects the target-supported sandbox substrate: Docker supports `"sprites"` or `"local"`, Fly supports `"sprites"`, and AWS supports `"aws"` or `"sprites"`. `sandbox.image` is the immutable rootfs pin used at boot; `sandbox.baseImage` records the digest-pinned build input; `sandbox.env` is non-secret runtime environment; `sandbox.secretEnv` lists org-wide secret names whose values are forwarded to every sandbox. `securityScreen` contains `backend: "proxy"`, a lowercase provider label, an HTTPS endpoint, and a `shadow` or `enforce` rollout. Its presence requires `secretEnv.core.SECURITY_SCREEN_PROXY_TOKEN`; absence keeps Auto on the built-in model classifier.
...
| Agent-computer image and credentials                                                       | local digest-pinned Docker image, or Fly app and scoped token for Sprites |        Fly app and scoped token | Lambda MicroVM image/version and execution role |
